  ...
  return (
    <div className="min-h-screen bg-gray-950 flex flex-col">
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        {/* ── Right content: map + audio ── */}
        <div className="flex-1 flex flex-col overflow-hidden">

          {/* Map — always visible, takes up most of the space */}
          ...
          {/* Audio clips — collapsible strip at the bottom */}
          <div className="border-t border-gray-800 shrink-0">
            <div className="px-4 py-2 flex items-center gap-2">
              <Mic className="w-3.5 h-3.5 text-gray-500" />
              <span className="text-gray-400 text-xs font-semibold uppercase tracking-wide">Audio Clips</span>
              <span className="text-gray-600 text-xs">({audioClips.length})</span>
            </div>
            {audioClips.length === 0 ? (
              <p className="text-gray-600 text-xs px-4 pb-3">No recordings yet — clips appear when a member activates tracking.</p>
            ) : (
              <div className="flex gap-3 px-4 pb-3 overflow-x-auto">
                {audioClips.map((clip) => (
                  <div
                    key={clip.id}
                    className="bg-gray-900 border border-gray-800 rounded-xl p-3 flex items-center gap-3 shrink-0"
                  >
                    <div
                    ...
                    <div className="min-w-0">
                      <p className="text-white text-xs font-medium">{clip.user.name}</p>
                      <p className="text-gray-500 text-[10px]">{new Date(clip.createdAt).toLocaleTimeString()}</p>
                    </div>
                    <audio controls src={clip.url} className="h-7 w-44" />
                  </div>
                ))}
              </div>
            )}
